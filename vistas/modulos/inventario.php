...
<section class="content">
    ...
    <div class="container-fluid">
        ...
        <div class="btn-agregar-inventario btnAgregar">
            <button type="button" class="btn btn-info btn-sm mb-4" data-toggle="modal" data-target="#modal-gestionar-inventario" data-dismiss="modal"> <i class="fas fa-plus-square"></i> Agregar Inventario</button>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        var table = $("#tablaInventario").DataTable({
            ...
            "columnDefs": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        return "<center>" +
                            "<button type='button' class='btn btn-primary btn-sm btnEditar' data-toggle='modal' data-target='#modal-gestionar-inventario'> " +
        ...
        $(".btn-agregar-inventario").on('click', function() {
            accion = "registrar";
        });

        $('#tablaInventario tbody').on('click', '.btnEliminar', function() {
            var data = table.row($(this).parents('tr')).data();

            var numero_inventario = data["numero_inventario"];
            ...
            swal.fire({
                ...
                title: "¡CONFIRMACION!",
                text: "Seguro que desea eliminar el inventario?",
                icon: 'warning',
        ...
        $('#tablaInventario tbody').on('click', '.btnEditar', function() {

            var data = table.row($(this).parents('tr')).data();
            accion = "actualizar";

            $("#txtinventario").val(data["numero_inventario"]);
            $("#txtfecha").val(data["fecha"]);
            $("#txtusuario").val(data["idusuario"]);
            $("#txtteclado").val(data["idteclado"]);
            $("#txtmouse").val(data["idmouse"]);
            $("#txtmonitor").val(data["idmonitor"]);
            $("#txtequipo").val(data["idequipo"]);
            $("#txtimpresora").val(data["idimpresora"]);
            $("#txttelefono").val(data["idtelefono"]);
        ...
        $("#btnGuardar").on('click', function() {
            ...
            var numero_inventario = $("#txtinventario").val(),
                // fecha = new Date().toISOString().slice(0, 19).replace('T', ' '),
                fecha = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, ''),
                idusuario = $("#txtusuario").val(),
                idteclado = $("#txtteclado").val(),
                idmouse = $("#txtmouse").val(),
                idmonitor = $("#txtmonitor").val(),
                idequipo = $("#txtequipo").val(),
                idimpresora = $("#txtimpresora").val(),
            ...
            datos.append('idteclado', idteclado);
            ...
            swal.fire({
                title: "¡CONFIRMAR!",
                text: "¿Está seguro que desea registrar el inventario?",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (resultado.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(respuesta) {
                            ...
                            $("#modal-gestionar-inventario").modal('hide');
