...
<section class="content">
    ...
    <div class="container-fluid">
        ...
        <div class="btn-agregar-equipo btnAgregar">
            <button type="button" class="btn btn-info btn-sm mb-4" data-toggle="modal" data-target="#modal-gestionar-equipo" data-dismiss="modal"> <i class="fas fa-plus-square"></i> Agregar Computadora</button>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        var table = $("#tablaEquipos").DataTable({
            ...
            "columnDefs": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        return "<center>" +
                            "<button type='button' class='btn btn-primary btn-sm btnEditar' data-toggle='modal' data-target='#modal-gestionar-equipo'> " +
        ...
        $(".btn-agregar-equipo").on('click', function() {
            accion = "registrar";
        });
        ...
        $('#tablaEquipos tbody').on('click', '.btnEliminar', function() {
            ...
            swal.fire({
                ...
                if (resultado.value) {
                    ...
                    // alert("no se modifico la equipo");
                }
        ...
        $('#tablaEquipos tbody').on('click', '.btnEditar', function() {
            ...
            $("#txtDisco").val(data["disco_duro"]);
            $("#txtCPU").val(data["procesador"]);
            $("#txtRAM").val(data["ram"]);
            $("#txtTipo").val(data["idtipo_equipo"]);
        ...
        $("#btnGuardar").on('click', function() {
            ...
            var idequipo = $("#idEquipo").val(),
                numero_inventario = $("#txtInventario").val(),
                numero_serie = $("#txtSerie").val(),
                marca = $("#txtMarca").val(),
                disco_duro = $("#txtDisco").val(),
                procesador = $("#txtCPU").val(),
                ram = $("#txtRAM").val(),
                idtipo_equipo = $("#txtTipo").val();
            ...
            datos.append('disco_duro', disco_duro);
            datos.append('procesador', procesador);
            datos.append('ram', ram);
            datos.append('idtipo_equipo', idtipo_equipo);
            datos.append('estado', estado);
            datos.append('accion', accion);

            swal.fire({
                title: "¡CONFIRMAR!",
                text: "¿Está seguro que desea registrar el equipo?",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (resultado.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(respuesta) {
                            ...
                            $("#modal-gestionar-equipo").modal('hide');
